...
CursorGeometry compute_cursor_geometry(size_t base_column, size_t character_count,
...
size_t compute_rendered_rows(size_t base_column, size_t character_count,
    ...
    return compute_render_end_geometry(base_column, character_count, columns).row +
           1;
}

void reset_input_render_state() {
    g_input_render_state = {};
}
...
void update_input_render_state(size_t header_display_width, size_t input_length,
...
RenderMetrics measure_render_state(size_t columns) {
    ...
    metrics.input_rows = compute_rendered_rows(
        prompt_prefix_display_width, g_input_render_state.input_length, columns);
    metrics.total_rows = metrics.header_rows + metrics.input_rows;
    metrics.cursor_row = compute_cursor_geometry(
...
std::string clear_previous_prompt_block(size_t old_columns, size_t new_columns) {
    ...
    return clear_render_block(new_metrics.cursor_row + new_metrics.header_rows,
                              std::max(old_metrics.total_rows,
                                       new_metrics.total_rows));
}
...
std::string build_prompt(const shell::ShellState &state) {
    const std::string header = build_prompt_header(state);
    reset_input_render_state();
    update_input_render_state(display_width(header), 0, 0, get_terminal_columns());
    return header + "\n" + build_prompt_prefix();
}
...
void redraw_input_line(const shell::ShellState &state, const std::string &line,
    ...
    const bool terminal_resized = columns != g_input_render_state.terminal_columns;
    const bool redraw_full_prompt = full_prompt || terminal_resized;
    const std::string header =
        redraw_full_prompt ? build_prompt_header(state) : "";
    const std::string prefix =
        redraw_full_prompt ? header + "\n" + build_prompt_prefix()
                           : build_prompt_prefix();
    ...
    const CursorGeometry end = line.empty()
                                   ? CursorGeometry{}
                                   : compute_render_end_geometry(
                                         prompt_prefix_display_width, line.size(),
                                         columns);
    const CursorGeometry cursor_geometry = compute_cursor_geometry(
        prompt_prefix_display_width, clamped_cursor, columns,
        clamped_cursor == line.size());

    std::string frame;
    if (full_prompt) {
        frame = "\r\033[2K";
    } else if (terminal_resized) {
        frame = clear_previous_prompt_block(g_input_render_state.terminal_columns,
                                            columns);
    } else {
    ...
    const size_t header_display_width =
        redraw_full_prompt ? display_width(header)
                           : g_input_render_state.header_display_width;
    update_input_render_state(header_display_width, line.size(), clamped_cursor,
                              columns);
}
